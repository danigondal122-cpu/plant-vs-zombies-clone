...

{
  ...
  window.DataManager = {
    XHR: function () {
      if (!(navigator.onLine && /(http:\/\/)|(https:\/\/)/.test(location.href))) {//online check if browser is connected to interents
        return "local";
      }
    ...
    CheckAchievement(type, ...arr) {
      ...
      let ps = function (name, ach, type = $__language_Array__["95d001bcead770c985b0a185c2599b35"]) {//obj[key]
        PlaySubtitle(`${$__language_Array__["02717ef3f8b4f0a331beec2c48bb8959"][0]}${type}${$__language_Array__["02717ef3f8b4f0a331beec2c48bb8959"][1]}${name}`);//Achivement unlocked
        oSym.addTask(500, function () {
          PlaySubtitle();///clears subtitles
        });
        data[ach[0]] = ach[1];//achievements dynamic update
      };

      let chk = function (cname, name, value, fun) {
        if (!$User.Achievement[name] && fun()) {//if achievement not unlocked
          ps(cname, [name, value]);
        }
      };
      ...
      switch (type) {//level
        case "lvl":
          let ls = arr[0];//get first argument //must be completed levels
          let winnum = ls.length;//levels
    ...
    LoginTip() {
      ...
      let buttonCssText = `
        ...
        `;
      let shade = NewEle("", "div", `position:absolute;top:0;left:0;width:1920px;height:1080px;`, {}, EDAll);
