...
var CPlants = NewO({
  ...
  */
  ...
  GetDX: self => -Math.floor(self.width * 0.5),//center plant horizontally on tile //- for left
  GetDY: (R, C, arg) => arg[0] ? -21 : -15, // shift plant according to cell vertically depend on argument
  //返回植物底座相对格子中点的偏移量，默认可根据是否种在花盆容器中调整
  GetDBottom: self => self.height,//eqal height
  ...
  getTriggerRange: (R, LX, RX) => [[LX, oS.W, 0]],//LX is in parametr, oS islike global object that return width of screen in pixels
 //2d array
  //传递触发器行上下限,返回格式是[小数行，大数行]
  getTriggerR: selfR => [selfR, selfR],//return array //duplicate variable in array

  InitTrigger(self, id, R, C, AttackedLX, AttackedRX) {
    //初始化植物自身触发器并注册全局触发器
    let trigger = {};
    let [minR, maxR] = self.getTriggerR(R);//It's used to ensure consistent access to the plant object in all inner functions or event handlers.

    for (; minR <= maxR; minR++) {//the loop always runs once
      //逐行注册全局触发器
      let _trigger = trigger[minR] = self.getTriggerRange(minR, AttackedLX, AttackedRX);

      oT.add(minR, _trigger, id);//oT gloval registeration
    }

    self.oTrigger = trigger; //add to self
  },
  ...
  EName: "oLawnCleaner",//internal name
  CName: $__language_Array__["e62c54032268681c517a5edd6264f0da"],//display name
  width: 71,//dimension of plant  for drwaing and positioning
  height: 57,
  canShovel: false,//shovel 
  beAttackedPointL: 0,//where it can be attacked
  beAttackedPointR: 60,// 0-60 range in right direction
  PicArr: ["images/interface/LawnCleaner.png", "images/Card/LawnCleaner.webp", 'images/Plants/SporeShroom/Effect.webp'],//image path
  AudioArr: ["lawnmower"],//file
  NormalGif: 0,//index
  CardGif: 1,//index for picarray maybe
  canEat: 0,//can zombies eat
  Stature: 1,//type
  HP: Infinity,

  Birth(X, Y, R, C, plantsArg) {
    //植物初始化方法
    let self = this,//this context of object //lock to current object configurations
        id = "P_" + Math.random(),//get a random id for plant
        pixelLeft = X + self.GetDX(self),//left pixel position
        //默认植物相对于FightingScene左侧的距离=格子中点坐标-0.5*植物图像宽度
    pixelTop = Y + self.GetDY(R, C, plantsArg) - self.height,//top pixel
        //默认植物顶部相对于FS顶部的距离=格子中点坐标+底座偏移-植物身高
    ele = NewEle(null, "div", "position:absolute;"); //new div for lawn mover
    self.id = id;
    self.pixelLeft = pixelLeft;
    self.pixelRight = pixelLeft + self.width;
    self.pixelTop = pixelTop;
    ...
    self.zIndex += 3 * R;//add zindex for row
    self.PicArr = self.PicArr.slice();//create a copy //does not affect original array复制一份数组，避免中途更改PicArr
    //初始化随机化图片

    for (let index in self.PicArr) {
      if (self.PicArr[index] && !/base64/.test(self.PicArr[index])) { //// Iterate over each image in PicArr and randomize it unless it's a base64-encoded string.
        self.PicArr[index] = RandomPic(self.PicArr[index], false, true);//add in array
      }
    }

    ele.addEventListener("unload", self.RemoveRandomPic, {//// Attach an event to remove the image from memory (cleanup) when the element is destroyed.
      once: true
    });
    $P[id] = self; // Store the plant in the global plant pool using its unique ID, so it can be referenced, updated, or removed.


    NewEle(`${id}_Shadow`, 'div', self.getShadow(self), {//// Add a div for the plant's shadow using calculated offset style (from getShadow). // Adds realism and depth visually.
      className: 'Shadow'
    }, ele); //绘制植物影子

    NewImg(0, self.PicArr[self.NormalGif], null, ele); // Insert the plant's main body image from the `PicArr` at index `NormalGif`.

    self.InitTrigger(self, id, self.R = R, self.C = C, self.AttackedLX = pixelLeft + self.beAttackedPointL, //植物左检测点
    self.AttackedRX = pixelLeft + self.beAttackedPointR // Register the plant’s position and attackable area (left/right detection points).
// Useful for collision detection with zombies.
    );


    self.BirthStyle(self, id, ele, { // Apply position and z-index CSS to the element.
      left: pixelLeft + "px",
      top: pixelTop + "px",
      zIndex: self.zIndex
    });
    ...
    oGd.add(self, `${R}_${C}_${self.PKind}`);// Add this plant to the game's logical grid (oGd), marking its location.

    self.PrivateBirth(self); // Optional post-setup (like spawn animation, glow, particle effects).
    return self;//return refernce
  },
...
oPeashooter = InheritO(CPlants, {
  ...
  NormalAttack() {
    let self = this,
        id = self.id,//get id
        dom = $(id);
    dom.childNodes[1].src = self.PicArr[self.AttackGif];//change to attack frame
    oSym.addTask(100, _ => {
      $(id) && (dom.childNodes[1].src = self.PicArr[self.NormalGif]);
    });
    oSym.addTask(60, _ => {
      PlayAudio(['throw', 'throw2'][Math.round(Math.random() * 1)]);
      CustomBullet(oPeaBullet, [self.PicArr[self.BulletGif]], self.AttackedLX, self.pixelTop + 15, self.R);//Custom bullet pass arguments to constructor method
    });
  }

}),
    oWitchHazel = InheritO(CPlants, {
  EName: "oWitchHazel",
  CName: $__language_Array__["e02b7d4c9d4a3a0b8d7f2ac3e15b9b14"],
  width: 120,
  height: 105,
  beAttackedPointR: 40,
  SunNum: 125,
  ...
  ProduceGif: 3,
  PicArr: function () {
    let a = "images/Plants/witchHazel/";
    return ["images/Card/witchHazel.png", a + "0.png", a + "idle.gif", a + "idle.gif", a + "lightning1.png", a + "lightning2.png"];
  }(),
  Tooltip: $__language_Array__["5a1c8a8c3de64f6d8b3a9e4e0bcd293e"],
  Story: $__language_Array__["9b6c62f90d0b4e5a8ecce234ad2c3f27"],
  ...
  PrivateBirth: function (self) {
     ...
     console.log(a)
      let id = a.id,
          ele = $(id);
         a.ElecPic = [new Image(), new Image()];
let loadCount = 0;

[a.PicArr[a.LightningGif], a.PicArr[a.LightningGif + 1]].forEach((src, i) => {
  a.ElecPic[i].onload = () => {
    loadCount++;
    console.log("WitchHazel lightning loaded:", src);
  };
  a.ElecPic[i].onerror = () => {
    console.error("Failed to load WitchHazel lightning image:", src);
  };
  a.ElecPic[i].src = src;
});
...
a.Conjure()
            oSym.addTask(825,fun , [X, Y]); //35s产一次
          }
        }));
      }, [GetX(a.C) + 75, GetY(a.R)]);
   ...
   Conjure() {
    ...
    //if we know two points we can make a vector, and can perform calculations on that vector to get angle and magnitude and addition etc
    function drawALink(ZX, pixelTop, oldCanvas = null) {//link from zombie to target plant
      //console.log(R,C);
      let pos = [GetX(obj.C), GetY(obj.R) - 20];//zombie position //ZX is x coordinate of zombie on screen
      let currentPos = pos;
      let targetPos = [ZX,pixelTop+20];//X abd Y coordinatw ofplant  //target position
//
      let theta = Math.atan2(targetPos[1] - currentPos[1], targetPos[0] - currentPos[0]);//y,x get and angle in terms of vector
      let delta = [Math.cos(theta), Math.sin(theta)];//cos tell how much to move horizontally to target , sin tell how much to move vertically per terget
      //
      let canvas = oldCanvas !== null && oldCanvas !== void 0 ? oldCanvas : NewEle("Canvas_Magician" + Math.random(), "canvas", "position:absolute;z-index:" + 3 * (oS.R + 1) + ";pointer-events:none;left:0;top:0;widht:100%;height:100%;" + (!$User.LowPerformanceMode ? "filter:brightness(220%);" : ""), {
        width: 900,
        height: 600
      }, FightingScene);//make canvas for lightening //fightening scene is a wrapper for these effect
      let ctx = canvas.getContext("2d");
      let kk = 0;

      function RotatePaint(pic, x, y, width, height, rotate = 0, mirror = 0) {//picture,x and y top left coordinate of image where to draw, rotation angle, mirror 0 is normal else horizontally
        let dist = [x + width / 2, y + height / 2];//center of image //origin to rotate
        ctx.save(); // save canvas state

        ctx.translate(dist[0], dist[1]); //canvas origin to image center

        ctx.rotate(rotate); // rotate

        ctx.translate(-dist[0], -dist[1]); // back to its location
        ...
        ctx.fillStyle = ["#000", "#fff"][kk ^= 1];//xor flip flop //ctx.fillRect(x,y,width,height);

        ctx.drawImage(pic, x, y, width, height); // 坐标参考还原

        ctx.restore(); // restore to orginal state before this save
      }

      let distance = Math.sqrt(Math.pow(targetPos[1] - currentPos[1], 2) + Math.pow(targetPos[0] - currentPos[0], 2));//distance between two points eucalidian formula
      let times = Math.max(2, Math.floor(distance / 80));//how many times segment make up the distance
      let k = distance / times;//make each segment equal
      let picPos = [];

      while (times-- > 0) {//post so while its greater than 0
        let randLength = k;//each segment length
        distance -= randLength;//- from distance draw each segment
        picPos.push({
          x: currentPos[0],//starting coordinate
          y: currentPos[1],//starting coordinate of each segement
          width: randLength,// how long segment is
          rotate: theta//angle to rotate
        });
        currentPos[1] += delta[1] * randLength;//move by offset from 1sst segment according to sin calculations
        currentPos[0] += delta[0] * randLength;//move by offset from 1st segment according to cos calculations
      }

      for (let t = 0; t < picPos.length; t++) {//make t for pic pos length
        let i = picPos[t]; //get first segment
       //offset vector
        let a = [-i.width / 2 + 10 * delta[1], -37.5 / 2 - 10 * delta[0]];//37.5 is half of sprite height
    ...
    if (!$P[id]) return;//if zombie does not exist return
    let arrZombies = obj.getZombies();//get all plants
   

    if ( arrZombies.length > 0) {//if all conditions are true
      //判定是否释放膜法
      ...
      oSym.addTask(300, _ => {
        ...
        //300
        //随机变一株植物为挨炮
        let aZombie = arrZombies.random(),//pick a random plant
            {
          ZX,
          pixelTop
        } = aZombie;//get plant row and column
        console.log(ZX,pixelTop)

        if ((_$Z$aZombie$id = $Z[aZombie.id]) !== null && _$Z$aZombie$id !== void 0 && $P[id]) {//plant.immediatly instant plant and zombie exist
          aZombie.getRaven();//kill plant with ticket
         
          let canvas = drawALink(ZX, pixelTop),
              ctx = canvas.getContext("2d");

          if (!$User.LowPerformanceMode) {//skip this if user has low graphics
            for (let i = 0; i < 3; i++) {//draw ligthening 3 times in quick succession
              oSym.addTask(2 * i, () => {
                if (!$(canvas.id)) {
                  return;
                }
                ...
                drawALink(ZX, pixelTop, canvas);
              });
            }
          }

          let img_index = Math.floor(Math.random() * 4);//randome 0-3 range
          ...
          // oSym.addTask(80, ClearChild, [elecEffect]);
          {
            ...
            oEffects.Animate(canvas, json, 1 / oSym.NowSpeed, "linear", ClearChild, 0.3 / oSym.NowSpeed);//fade out the canvas and remove the element
          }
          ;
          // oEffects.ImgSpriter({
...
oSunShroom = InheritO(CPlants, {
  ...
  TriggerCheck: function (zombie, direction) {
    var self = this,
        id = self.id;
    let dom = $(id);
    let pic = dom.childNodes[1]; //the 2nd element of plant element is img

    if (zombie.PZ && !zombie.isPuppet && Math.abs(zombie.ZX - self.MX) < 121 && zombie.beAttacked) {//ZX-MX distance horizonally from plant
      self.ArZ.push(zombie.id);//store zombies in range

      if (!self.Cry) {//if plant is not cry
        self.Cry = 1;
        pic.src = self.PicArr[4];
        self.CryCheck(id);
      }
    } else {
      if (zombie.R == self.R && !self.Cry && !self.Attacking && zombie.Altitude > 0 && zombie.Altitude < 3) {//row same no cry not attacking //altiftude ground
        self.NormalAttack();//attack
      }
    }

    if (self.Cry == 1 && pic.src != self.PicArr[4]) {//crying
      pic.src = self.PicArr[4];//change pic to cry
    } else if (!self.Attacking && self.Cry == 0 && pic.src != self.PicArr[2]) {//zombie not in row //no cry
      pic.src = self.PicArr[2];//change src of pic
    }
  },


  PrivateBirth: function (c) { //private function //c is plant
    var b = c.AttackedLX,
        a = b - 40;//from attacked hotbox 40pixel away
    c.BulletClass = NewO({//new object
      X: b,//X ccordinate is that of hitbox
      R: c.R,// Row of plant
      pixelLeft: a,//pixel left is a
      F: oGd.MB2//undefined
    });
    c.BulletEle = NewImg(0, "images/Plants/ShroomBullet.png", "left:" + a + "px;top:" + (c.pixelTop + 110) + "px;visibility:hidden;z-index:" + (c.zIndex + 2));//plant z index +2
    c.MX = b + 9;//increate MX according to b
  },


  PrivateDie: function (a) {
    a.BulletEle = null;//set element to null
  },

  NormalAttack: function () {//attack if zombie hit than remove else move +=5
    var c = this,
        a = c.id,
        d = "SSB" + Math.random(),//d
        b = c.AttackedLX;
    EditEle(c.BulletEle.cloneNode(false), {//dont copy children cause they dont exist anyway
      id: d
    }, 0, EDPZ);//EDPZ is lobal wrapper
    PlayAudio("puff");
    $(a).childNodes[1].src = c.PicArr[3];//change to attack animation
    ...
    c.Attacking = 1;//is attacking
    oSym.addTask(40, function (g) {//after 40ms
      var f = $(g);//get plant element
      f && (f.childNodes[1].src = c.PicArr[c.NormalGif]);//Normal Gif is 2
    }, [a]);

    oSym.addTask(10, function (g, e) {
      var f = $(g);//
      f && SetVisible(f);//make bullet element visible
      oSym.addTask(130, function (h) {
        var i = $P[h];//add in $P global plant manager
        i && (i.Attacking = 0); //set attack to 0
      }, [e]);
    }, [d, a]);
  },
  ...
  CryCheck: function (id) {
    oSym.addTask(140, function (pid) {
      var self = $P[pid],//self is plant in golobal plant maanger
          len,
          ArZ,//zombies interacted with plants
          zombie;

      if (self) {
        len = (ArZ = self.ArZ).length;

        while (len--) {//for each post
          if (!(zombie = $Z[ArZ[len]]) || !zombie.PZ || Math.abs(zombie.ZX - self.MX) > 120) {//zombie is out of range
            ArZ.splice(len, 1);//cut the zombie //if zombie is gine
          }
        }

        ArZ.length ? self.CryCheck(pid) : (self.Cry = 0, $(pid).childNodes[1].src = self.PicArr[2]);//recursive check 140 ticks
      }
    }, [id]);
  }
}),
  ...
  PrivateBirth: function (o) {
    oSym.addTask(80, function (id) {
      ...
      if (obj) {
        ...
        let R = o.R,//get current row of obj
            floorR = R > 3 ? R - 2 : 1,
