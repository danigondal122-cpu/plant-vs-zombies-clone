...
SetAlpha = (ele, alpha) => ele.style.opacity = alpha,
  ...
  //创建新元素
  const ele = $n(tag);//new element with tag name
  ele.id = id;//assign id
  ele.style.cssText = cssText;//css text
  Object.assign(ele, properties);//assign properties
  wrap && wrap.appendChild(ele);//append element
  return ele;
},
...
const EditCompositeStyle = ({
  ...
  if (option === 2) { //return style style 
...
IsMobile = _ => Mobile,
canXHR = navigator.onLine && /(http:\/\/)|(https:\/\/)/.test(location.href),//if user is online and 
RandomPic = function (l, a, b, callback = null) {//if no xhr
  let link = l+(!canXHR?`?${oSym.Now}`:"");//append current tick to cache image and make it unique //bust the cache by making images unique
  if (callback) {
    callback(link);
  }
