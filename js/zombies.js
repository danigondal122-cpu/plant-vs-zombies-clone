...
var CZombies = NewO({
  ...
  StaticGif: 2,//walking goif
  AttackGif: 3,//attack gif
  LostHeadGif: 4,//head
  LostHeadAttackGif: 5,//head gif
  HeadGif: 6,//head gif
  DieGif: 7,//head gif
  width: 166,
  height: 144,
  beAttackedPointL: 82,//left hitbox
  beAttackedPointR: 156,//right hotbox
  BreakPoint: 90,//armor break like losing helmet
  Ornaments: 0,//healt or shield
  OrnHP: 0,//armor hp
  OSpeed: 1.6,//original spped
  ...
  beAttacked: 1,//1 can be hit //0 immune
  //是否可以被攻击，可用于粗略判断僵尸是否垂死
  isAttacking: 0,//attacking
  isGoingDie: 0,//dying
  //僵尸是否垂死
  OAttack: 100,
  Attack: 100,
  LivingArea: 1,//type of tile zombie is currently on
  //目前所在格子的类型
  Altitude: 1,//zombiw grid layer underground, ground, air
  // //海拔位置：-1潜地 0潜水 1行走 2跳跃 3植物子弹无法到达的高空飞行
  WalkDirection: 0,//0 left 1 right
  //僵尸行走方向，0表示向左，1表示向右
  FreeSetbodyTime: 0,//freeze time
  FreeFreezeTime: 0,//time remaininng in frozen state
  FreeSlowTime: 0,//in slow state
  //用于标记僵尸是否处于减速状态
  FreeVertigoTime: 0,//time stunned / dizzy
  FreeExcitedTime: 0,//speed up time remaining
  isFloating: 0,//flag for special effect like knock back
  //标记僵尸是否处于浮空状态，即是否受战术风扇影响
  isCounted: 1,//zombie count in level prohress
  //是否需要纳入死亡统计
  isPuppet: 0,//fake zombies
  //是否是傀儡(非僵尸的僵尸)
  CanDiaplay: 1,//remdering should be visible type
  WaterShadowGif: null,
  Almanac: {
    Dom: null,
  ...
  AccessiblePath: (() => {//zombie rule while walking on terrain
    //可由某道路转向某道路
    let arr = {};
    arr["1_3"] = 1;//can walk beteen terrain 1 to 3

    for (let i in arr) {
      arr[i.reverse()] = 1;
    }

    for (let i = 0; i < 4; i++) {//terrain to itslef is always passable
      arr[`${i}_${i}`] = 1;
    }

    arr[`2_2`] = 0; //默认不走水路 //water to water is not //zombie just cannot start to water immediately

    return arr;
  })(),

  //获取卡片图片
  GetCardImg() {//get zombie card image
    let self = this;
    return "images/Card/" + self.EName.substr(1) + ".webp";
  },

  //判定僵尸能否走某行
  CanPass(R, LF) {//row and LF
    let self = this;
    return LF && self.AccessiblePath[oGd.$LF[R] + "_" + LF];//checl if zombie can pass the rowby looking its lane type
  },

  //检查当前格子类型不同切换事件
  ChkCell_GdType(self) {//zomnir continuously check the cell they are walking on
    let R = self.R;
    let C = GetC(self.ZX + (self.beAttackedPointR - self.beAttackedPointL) / 2);//get column they are walkin on
    let gdType = oGd.$GdType[R][C];//cell unit type

    if (self.LivingArea != gdType) {
      if (gdType == 1 || gdType == 3) {//cell type gorund or roof
        if (self.LivingArea == 2) {//if its water
          self.SetWater(0);//water to 0 float out og water
        }
      }
    ...
    if (gdType == 2) {
      if (self.DivingDepth != oGd.$WaterDepth[R][C]) {//if water depth is not equal to that of cell
        self.SetWater(oGd.$WaterDepth[R][C]);//set it to water depth
      }
    }
  },
  ...
  GoingDieHeadNew(id, PicArr, self, config = {}) {//it's a prototype method
    //通过旋转图片来达到掉头
    let bca,
...
oZombie = InheritO(OrnNoneZombies, {
  ...
  PicArr: (a => ["", "", a + "Zombie.webp", a + "ZombieAttack.webp", a + "ZombieLostHead.webp", a + "ZombieLostHeadAttack.webp", a + "ZombieHead.png", a + "ZombieDie.webp", a + "1.webp", 'images/Zombies/BoomDie.webp'])("images/Zombies/Zombie/"),
...
oChicken = InheritO(OrnNoneZombies, {
  EName: "oChicken",
  CName: $__language_Array__["c867bd4b2d9e4a9bed4df55d9da81877"],
  ...
  getShadow: self => "left:75px;top:" + (self.height - 25) + "px;",
    PicArr: (a => ["", "", a + "Zombie.webp", a + "ZombieAttack.webp", a + "ZombieLostHead.webp", a + "ZombieLostHeadAttack.webp", a + "ZombieHead.webZombieHead.png", a + "ZombieDie.webp", a + "1.webp", 'images/Zombies/BoomDie.webp'])("images/Zombies/Zombie/"),

   GoLeft(o, R, arR, i) {
    return 1; // just stay idle
  },
  GoRight(o, R, arR, i) {
    return 1; // idle
  },
  ...
  PicArr: function () {
    ...
    return ["", "", b + "ConeheadZombie.webp", b + "ConeheadZombieAttack.webp", a + "ZombieLostHead.webp", a + "ZombieLostHeadAttack.webp", a + "ZombieHead.png", a + "ZombieDie.webp", a + "Zombie.webp", a + "ZombieAttack.webp", b + "1.webp", 'images/Zombies/BoomDie.webp'];
  }(),
  ...
  PicArr: function () {
    var b = "images/Zombies/BucketheadZombie/",
        a = "images/Zombies/Zombie/";
    return ["", "", b + "BucketheadZombie.webp", b + "BucketheadZombieAttack.webp", a + "ZombieLostHead.webp", a + "ZombieLostHeadAttack.webp", a + "ZombieHead.png", a + "ZombieDie.webp", a + "Zombie.webp", a + "ZombieAttack.webp", b + "1.webp", 'images/Zombies/BoomDie.webp'];
  }()
}),
    //森林僵尸从以下开始
oNewspaperZombie = InheritO(OrnIIZombies, {
  ...
  PicArr: (() => {
    ...
    return ["", "", a + "Zombie.webp", a + "ZombieHead.png", a + "ZombieDie.webp", a + "1.webp", 'images/Zombies/BoomDie.webp'];
  })(),
  ...
  PicArr: function () {
    ...
    return ["", "", a + "FootballZombie.webp", a + "Attack.webp", a + "LostHead.webp", a + "LostHeadAttack.webp", "images/Zombies/Zombie/ZombieHead.png", a + "Die.webp", a + "OrnLost.webp", a + "OrnLostAttack.webp", a + "1.webp", 'images/Zombies/BoomDie.webp'];
  }(),
  getShadow: function (a) {
...
oCaskZombie = function () {
  ...
  PicArr: function () {
    ...
    return ["", "", a + "Zombie.webp", a + "ZombieAttack.webp", a + "ZombieLostHead.webp", a + "ZombieLostHeadAttack.webp", a + "ZombieHead.png", a + "ZombieDie.webp", a + "1.webp", a + "ZombieThrowing.webp", a + "ZombieLand.webp", a + 'BoomDie.webp'];
  }(),
  AudioArr: function () {
    let arr = [];
...
oMembraneZombie = InheritO(OrnNoneZombies, {
  ...
  PicArr: function () {
    ...
    let b = ["", "", a + "Zombie.webp", a + "ZombieAttack.webp", a + "ZombieLostHead.webp", a + "ZombieLostHeadAttack.webp", a + "ZombieHead.png", a + "ZombieDie.webp", a + "1.webp", a + 'conjure.webp', a + 'ZombieLostHeadConjure.webp', a + 'effect.png', 'images/Zombies/BoomDie.webp', a + "lightning1.png", a + "lightning2.png"];
    let c = [];

    for (let i = 1; i <= 4; i++) {//i must be 4
      c.push(a + "effect" + i + ".webp");//copy al 4 effects in array
    }
  ...
  BirthCallBack(self) {
    self.ElecPic = [new Image(), new Image()];//preload images when zombie spawns
    self.ElecPic[0].src = self.PicArr[self.LightningGif];
    self.ElecPic[1].src = self.PicArr[self.LightningGif + 1];
    OrnNoneZombies.prototype.BirthCallBack(self);//call the prototype of spwaning
  },
  ...
  GoLeft(o, R, arR, i) {
    ...
    var Speed,
        ...
        id = o.id;
    !(o.FreeFreezeTime || o.FreeSetbodyTime) ? (o.beAttacked && !o.isAttacking && o.JudgeAttack(), //未临死，未攻击，进行攻击判断
    !o.isAttacking ? (AttackedRX = o.AttackedRX -= Speed = o.Speed * o.SpeedCoefficient) < -50 ? (arR.splice(i, 1), o.DisappearDie(), rV = 0) : ( //zombie has passed left screen boundary die else move left and conjure
  ...
  Conjure() {
    ...
    function drawALink(R, C, oldCanvas = null) {//link from zombie to target plant
      //console.log(R,C);
      let pos = [obj.ZX, obj.pixelTop + 50];//zombie position //ZX is x coordinate of zombie on screen
      let currentPos = pos;
      let targetPos = [GetX(C), GetY(R) - 20];//X abd Y coordinatw ofplant  //target position
//
    ...
    if (!$Z[id]) return;//if zombie does not exist return
    let arrPlants = obj.getPlants();//get all plants
    arrPlants = arrPlants.filter(p => p.EName !== "oWitchHazel");
    obj.Pianyi++; //offset counter like a timer

    if (obj.AttackedLX <= oS.W + 80 && !obj.isAttacking && obj.beAttacked && obj.Pianyi >= 190 && arrPlants.length > 0) {//if all conditions are true
      //判定是否释放膜法
      obj.Pianyi = 0; //重置计数器 set to 0

      obj.isAttacking = 2; //标记正在施法，确保僵尸停止运动 casting state

      obj.EleBody.src = obj.PicArr[obj.ConjureGif];//set picture gif
      PlayAudio('conjure');
      oSym.addTask(300, _ => {
        var _$P$aPlant$id;
        ...
        //300
        //随机变一株植物为挨炮
        let aPlant = arrPlants.random(),//pick a random plant
            {
          R,
          C
        } = aPlant;//get plant row and column

        if (((_$P$aPlant$id = $P[aPlant.id]) === null || _$P$aPlant$id === void 0 ? void 0 : _$P$aPlant$id.Immediately) === false && $Z[id]) {//plant.immediatly instant plant and zombie exist
          aPlant.Die('JNG_TICKET_MembraneZombie');//kill plant with ticket
          CustomSpecial(oZombie, GetY(R), GetX(C));
          ...
          oSym.addTask(80, ClearChild, [elecEffect]);
          {
            ...
            oEffects.Animate(canvas, json, 0.3 / oSym.NowSpeed, "linear", ClearChild, 0.1 / oSym.NowSpeed);//fade out the canvas and remove the element
          }
          ;
          oEffects.ImgSpriter({
      ...
      $Z[id] && obj.beAttacked && (obj.isAttacking = 0, obj.EleBody.src = obj.PicArr[obj.NormalGif]));//normal walking sprite
    }
  },
  ...
  PicArr: function () {
    ...
    return ["", "", a + "Zombie.webp", a + "ZombieAttack.webp", a + "ZombieLostHead.webp", a + "ZombieLostHeadAttack.webp", a + "ZombieHead.png", a + "ZombieDie.webp", a + '1.webp', 'images/Zombies/BoomDie.webp'];
  }(),
  ...
  PicArr: function () {
    var a = "images/Zombies/SkatingZombie/",
        b = "images/Zombies/";
    return ["", "", a + "Zombie.webp", a + "ZombieAttack.webp", a + "ZombieLostHead.webp", a + "ZombieLostHeadAttack.webp", a + "ZombieHead.png", a + "ZombieDie.webp", 'images/Zombies/BoomDie.webp'];
  }(),
  getCharredCSS: _ => ({
    top: `45px`,
    left: `40px`
  ...
  //头的位置数据
  PicArr: (a => [a + "zombie.webp", a + "ZombieAttack.webp", a + "ZombieLostHead.webp", a + "ZombieLostHeadAttack.webp", a + "ZombieHead.png", a + "ZombieDie.webp", a + "zombie2.webp", a + 'filling.webp', 'images/Zombies/Imp/BoomDie.webp'])("images/Zombies/PushIceImp/"),
...
oSculptorZombie = InheritO(oZombie, {
  ...
  PicArr: (path => ['ZombieWalk.webp', 'ZombieAttack.webp', 'ZombieLostHeadWalk.webp', "ZombieLostHeadAttack.webp", "ZombieHead.png", "ZombieDie.webp", "ZombieStand.webp", 'ZombiePush.webp', 'ZombieLostHeadPush.webp'].map(s => path + s))("images/Zombies/SculptorZombie/").concat('images/Zombies/BoomDie.webp'),
