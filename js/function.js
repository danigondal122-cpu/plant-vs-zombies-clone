...
GroundOnmousemove1 = (event, plant = ArCard[oS.ChoseCard].PName) => {
  ...
  let AC = ArCard[oS.ChoseCard = index]; 
...
ShowWinItem = ele => {
  ...
  let scale = EditCompositeStyle({ //extract css value
    ele,
    targetFunc: 'scale'
  });
  ...
  if (scale !== '' && scale !== '1' && ele.tagName === 'IMG') {
    ...
    ele.onload = _ => {
      ...
      canvas.onclick = ele.onclick.bind(canvas);//binds onclick
...
oMiniGames = {
  ...
  */
  ...
  oStg: {
    ...
    oEffects = {
  AudioFadeOut(audio, targetVolume = 0, duration = 1, callback) {   //audio fades out from 1 to 0
  ...
  AudioFadeIn(audio, targetVolume = 1, duration = 1, callback) { //fade in from current level to 1
    if (typeof audio === 'string') {
  ...
  Particle({ //Particle System //coninuosly render particles
    style = "",
    height = 60,
    width = 60,
    objects = [],
    ...
    //准备工作
    const id = `BgParticle_${Math.random()}`; //creeate random ids
    let canvas = NewEle(id, 'canvas', style, { //add new element with id canvas and stule
      height: height,
      width: width,
      className: 'BgParticle'
    }, EDAll);//parent
    ...
    function RotatePaint(pic, x, y, width = -1, height = -1, rotate = 0, alpha = 1) { //pic , coordinates, scale, rotate and transparency
      let dist = [x + width / 2, y + height / 2];//center of image
      ctx.save(); // Save the state to avoid affecting other objects

      ctx.globalAlpha = alpha; // 
      ctx.translate(dist[0], dist[1]); //move origin to center of object

      ctx.rotate(rotate); //rotate canvas

      ctx.translate(-dist[0], -dist[1]); // move origin back

      if (width > 0) {
        ctx.drawImage(pic, Math.round(x), Math.round(y), width, height); //how large image should be when drawn
      } else {
        ctx.drawImage(pic, Math.round(x), Math.round(y));
      } // 坐标参考还原


      ctx.restore(); // restore canvas state to normal
    }

    canvasFunction(canvas);//passes canvas to callback  function

    for (let i = 0; i < objects.length; i++) { //update object properties 
      var _objects$i$alpha, _objects$i$rotate, _objects$i$width;

      if (oImage["TMP_" + objects[i].src]) {//cache
  ...
  */
  ...
  init({ //method
    ...
    this.__ContainerEle__ = $('dFlagMeterContent'); //__ __ internal/private
    this.__MeterEle__ = $('dFlagMeterBar');
    this.__HeadEle__ = $('dFlagHead');
    this.__MeterEle__.className = MeterType;//set classname
    this.__HeadEle__.className = HeadType;//ser className
    this.fullValue = fullValue;
    this.curValue = curValue;
    this.canMoveHead = canMoveHead;// When fullValue is 0, it must still be set (not skipped), because the animation logic depends on it.

    const val = (this.fullValue === 0 ? 0 : curValue / fullValue) * this.fullWidth; //get progress bar value according to percentage
    this.__MeterEle__.style.width = val + 'px';
    canMoveHead && (this.__HeadEle__.style.right = val - this.headWidth / 2 + 'px'); //move head with val
    return this;
  },
  ...
  update({
    ...
    if (curValue !== null && curValue !== void 0 && curValue <= this.fullValue) { //void 0 is undefined
      //更改记录值
      this.curValue = curValue; //更新进度条                fullValue可能直接为0

      const val = (this.fullValue === 0 ? 1 : curValue / this.fullValue) * this.fullWidth;//get percentage and than multiply by full
